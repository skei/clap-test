...
static
ssize_t read_vlv(FILE *f, size_t avail, uint32_t *value) {
  ssize_t ret;
  uint8_t buf;
...
static
ssize_t read_sysex_event(struct tinysmf_parser_ctx *ctx, FILE *f, size_t avail, int delta) {
  ssize_t res, ret;
  uint32_t length;
  uint8_t *buf;
  ret = 0;
...
static
void cook_meta_event(struct tinysmf_meta_event *ev) {
  switch (ev->meta_type) {
    case TINYSMF_META_TYPE_MIDI_CHANNEL:
      ev->cooked.midi_channel = ev->raw[0];
      break;
    case TINYSMF_META_TYPE_SET_TEMPO:
      ev->cooked.bpm = 60.0 / (((ev->raw[0] << 16) | (ev->raw[1] << 8) | ev->raw[2]) * 1e-06);
      break;
    default:
      break;
  }
...
static
ssize_t read_meta_event(struct tinysmf_parser_ctx *ctx, FILE *f, size_t avail, int delta) {
  ...
  ev.delta = delta;
  ev.meta_type = (tinysmf_meta_event_type_t)type;
  ev.nbytes = length;
  ret += length;
  ...
  if (length > 0) {
    if (length > avail || !(fat_ev = (tinysmf_meta_event*)malloc(sizeof(*fat_ev) + length + 1)))
      return -1;
    *fat_ev = ev;
    buf = (uint8_t *) &fat_ev[1];
    if (!fread(buf, length, 1, f)) {
      free(fat_ev);
      return -1;
    }
...
static
ssize_t midi_msg_size(uint8_t status_byte) {
...
static
ssize_t read_midi_event(struct tinysmf_parser_ctx *ctx, FILE *f, size_t avail, int delta, uint8_t *status_buf, uint8_t first_byte) {
  unsigned to_read;
  uint8_t *buf;
...
static
int read_track(struct tinysmf_parser_ctx *ctx, FILE *f, ssize_t avail) {
...
static
void cook_file_info(struct tinysmf_parser_ctx *ctx, struct header_chunk *hdr) {
  struct tinysmf_file_info *info = (tinysmf_file_info *) &ctx->file_info;
  info->format = (tinysmf_file_format_t)hdr->format;
  info->num_tracks = hdr->num_tracks;
  if (hdr->division & 0x80) {
    uint8_t *div = (uint8_t *) &hdr->division;
...
int tinysmf_parse_stream(struct tinysmf_parser_ctx *ctx, FILE *f) {
